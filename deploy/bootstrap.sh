...
echo "    source .env"
echo "    scp -i ~/.ssh/aro-demo-vm -P 2222 deploy/step1-vm/setup-app.sh cloud-user@127.0.0.1:~/"
echo "    ssh -i ~/.ssh/aro-demo-vm -p 2222 cloud-user@127.0.0.1 \\"
echo "      \"RHSM_USERNAME=\$RHSM_USERNAME RHSM_PASSWORD=\$RHSM_PASSWORD bash ~/setup-app.sh\""

echo ""
echo "# ============================================================"
echo "# STEP 2: OpenShift Service Mesh (OSSM 3.x — Ambient Mode)"
echo "# ============================================================"
...
echo "#"
echo "# DEMO TOGGLE — 'with mesh' (ambient, no VM restart needed):"
echo "#   oc apply -f deploy/argocd/application-ossm-config.yaml"
echo "#   # OR sync manually in ArgoCD UI"
echo "#   # Ambient mesh: NO VM restart needed. Label applied → ztunnel captures traffic immediately."
echo "#"
...
echo "#   # → prune removes the istio.io/dataplane-mode=ambient label from eshoplite-vm namespace"
echo "#"
